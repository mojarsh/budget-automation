...
import requests
import pytest
from budget_automation.starling import _clean_export, AccountOperations

class TestCleanExport:

    def test_pence_converted_to_pounds(self, raw_transaction_df):
        result = _clean_export(raw_transaction_df)
    ...
    def test_output_columns_are_correct(self, raw_transaction_df):
        expected_cols = [
            "transaction_id", "transaction_date", "outflow",
            "inflow", "category", "account", "reference", "status",
        ]
        result = _clean_export(raw_transaction_df)
