...
class AccountOperations:
    ...
    def account_uid(self) -> str:
        """
        Obtain Starling Bank account uid
        """
    ...
    def current_balance(self) -> int:
        ...
        """
        balance = requests.get(self.url + "accounts/" + self.account_uid + "/balance", headers=self.headers)
    ...
    def export_transactions(self, date: str) -> DataFrame:
        ...
        transactions = requests.get(
            self.url + "feed/account/" + self.account_uid + "/settled-transactions-between?" +
            "minTransactionTimestamp=" + date + "&"
            "maxTransactionTimestamp=" + datetime.utcnow().strftime(self.timestamp_format), headers=self.headers)
